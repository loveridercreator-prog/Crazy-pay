...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && urlPath === '/api/payment_gateways/evaluate') {
        let body = '';
        req.on('data', chunk => body += chunk);
        req.on('end', async () => {
            try {
                ...
                // Fetch dynamic tools from Firebase
                let tools = await firebaseRequest('admin_tools', 'GET');
                let allTools = [];
                if (tools) {
                    allTools = Object.keys(tools).map(key => ({
                        key: key,
                        toolName: tools[key].toolName || "",
                        gatewayType: tools[key].gatewayType || "UPI",
                        pairCategory: tools[key].pairCategory || "BOTH",
                        status: tools[key].status || "Active",
                        logoUrl: tools[key].logoUrl || ""
                    }));
                } else {
                    allTools = [
                ...
                // Filter only Active gateways of BUY or BOTH category
                let activeGateways = allTools.filter(t => t.status === "Active" && (t.pairCategory === "BUY" || t.pairCategory === "BOTH"));

                // Define backend limits configuration
                const gatewayLimits = {
                    "mobikwik": { min: 100, max: 50000 },
                    "freecharge": { min: 200, max: 15000 },
                    "freecharge gateway": { min: 200, max: 15000 },
                    "phonepe": { min: 100, max: 30000 },
                    "paytm business": { min: 500, max: 100000 },
                    "google pay": { min: 100, max: 25000 },
                    "super money": { min: 300, max: 20000 }
                };

                // Filter by transaction amount limits
                let matchedGateways = activeGateways.filter(g => {
                    const nameLower = g.toolName.toLowerCase();
                    let minLimit = 50; // default min limit
                    let maxLimit = 100000; // default max limit

                    // Match limit key by substring matching
                    for (const key of Object.keys(gatewayLimits)) {
                        if (nameLower.includes(key)) {
                            minLimit = gatewayLimits[key].min;
                            maxLimit = gatewayLimits[key].max;
                            break;
                        }
                    }

                    return parsedAmt >= minLimit && parsedAmt <= maxLimit;
                });
                ...
                res.writeHead(500, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ success: false, error: "Internal gateway evaluation error: " + err.message }));
            }
        });
        return;
    }
...
const atomicOrderLocks = new Set();
    ...
    if (req.method === 'POST' && urlPath === '/api/claim_order') {
        ...
        req.on('end', async () => {
            ...
            try {
                const { orderId, buyerPhone, amount, provider } = JSON.parse(body || '{}');
                if (!orderId || !buyerPhone || !amount) {
                    res.writeHead(400, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, error: "Missing required payload parameters" }));
                    return;
                }
                ...
                const orderData = await firebaseRequest(`p2p_orders/${orderId}`, 'GET');
                if (!orderData || (orderData.status !== 'AVAILABLE' && orderData.status !== 'pending' && orderData.status !== 'PENDING')) {
                    atomicOrderLocks.delete(orderId);
                    res.writeHead(409, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, code: "ORDER_UNAVAILABLE", error: "Order is no longer available" }));
                    return;
                }
                
                // Validate payload
                if (parseFloat(orderData.amount || orderData.displayAmount || 0) !== parseFloat(amount)) {
                    atomicOrderLocks.delete(orderId);
                    res.writeHead(400, { 'Content-Type': 'application/json' });
                    res.end(JSON.stringify({ success: false, code: "INVALID_AMOUNT", error: "Amount mismatch detected" }));
                    return;
                }

                // Update order in Firebase
                const fiveMinsFromNow = Date.now() + 600000; // 10 minutes TTL
                await firebaseRequest(`p2p_orders/${orderId}`, 'PATCH', {
                    status: "IN TRANSACTION",
                    expiry: fiveMinsFromNow,
                    expiry_time: fiveMinsFromNow,
                    buyerPhone: cleanPhone,
                    claimedAt: Date.now()
                });

                // Update buyer transaction history
                const buyTxRecord = {
                    id: orderId,
                    type: "Buy Order Lock",
                ...
                await firebaseRequest(`users/${cleanPhone}/transactions/tx_${orderId}`, 'PUT', buyTxRecord);
                await firebaseRequest(`users/${cleanPhone}/active_buy`, 'PUT', {
                    id: orderId,
                    phone: cleanPhone,
                    amount: parseFloat(amount),
                    provider: provider || "MobiKwik",
  ...
  let body = '';
    ...
    if ((req.method === 'GET' || req.method === 'POST') && urlPath === '/api/usdt/check_status') {
        ...
        const executeCheck = async (idVal) => {
            ...
            // Removed simulation block so USDT remains pending.
...
// Start the server-side autonomous balance poller job (runs every 10 seconds to prevent network rate-limiting/hang-ups)
setInterval(pollAndProcessAllUsers, 10000);
